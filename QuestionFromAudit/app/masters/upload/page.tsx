          ...
          for (const [id, questionRows] of groupedRows) {
            ...
            try {
              ...
              if (ITEM_TYPES_REQUIRING_OPTIONS.includes(itemType)) {
                // radio requires 1+ options (single option = acknowledgement/condition acceptance)
                // checkbox requires 2+ options
                const minOptions = itemType === "radio" ? 1 : 2;
                if (question.options.length < minOptions) {
                  reject(
                    `Question ${question.id}: ${itemType} type requires at least ${minOptions} option${minOptions > 1 ? "s" : ""} (found ${question.options.length})`
                  );
                  return;
                }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.success(`Master created! Dashboard URL: ${adminUrl}`, {
          duration: 8000,
        });
      }
    ...
    <div className="min-h-screen bg-base-200 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-base-100 rounded-xl shadow-lg p-8">
          ...
          <div className="mb-6">
            <label className="label" htmlFor="questionnaire-name">
              <span className="label-text font-medium">Questionnaire Name <span className="text-error">*</span></span>
            </label>
            <input
              id="questionnaire-name"
              type="text"
              placeholder="e.g., Pre-Operative Assessment 2026"
              value={name}
              ...
              className={`input input-bordered w-full ${nameTouched && !name.trim() ? "input-error" : ""}`}
            />
            {nameTouched && !name.trim() && (
              <p id="name-error" role="alert" className="text-error text-sm mt-1">
...
Q002,Who I Am,Personal Details,text-field,What is your name?,,patient_name,TRUE,,FALSE,,,`}
          ...
          {(!name.trim() || !file) && (
            <p className="text-base-content/50 text-sm text-center mt-2">
              {!name.trim() && !file
                ? "Enter a questionnaire name and upload a CSV to continue"
                : !name.trim()
                  ? "Enter a questionnaire name to continue"
