...
export default function AdminPageClient({ adminLinkId, initialData }: AdminPageClientProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [project, setProject] = useState<ProjectData | null>(initialData);
  ...
  // Filter suggestions (memoized to avoid recomputation on every render)
  const filteredSuggestions = useMemo(() => {
    return project?.suggestions.filter((s) => {
  ...
  }, [project?.suggestions, statusFilter, searchTerm]);

  // Count suggestions by status (memoized)
