...
/**
 * Data for a suggested new question (stored in component_changes JSONB)
 */
...
/**
 * A phantom question suggested by a trust user (not yet a real question).
 * Rendered inline in the question list with dashed border + NEW banner.
 */
export interface PhantomQuestion extends EditableQuestion {
  isPhantom: true;
  phantomSuggestionId: number;
  anchorQuestionId: number;
  phantomPosition: "before" | "after";
  submitterName: string;
}

/**
 * Union type for items rendered in the question list
 */
export type QuestionListItem = EditableQuestion | PhantomQuestion;
