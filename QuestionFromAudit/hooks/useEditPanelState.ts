...
export function useEditPanelState(
  ...
  const validate = useCallback((): ValidationErrors => {
    ...
    if (state.changes.content?.options) {
      ...
      if (answerType === "radio" || answerType === "checkbox" || answerType === "multi_select") {
        ...
        const minOptions = answerType === "radio" ? 1 : VALIDATION_LIMITS.MIN_OPTIONS_FOR_CHOICE;
        if (resultingCount < minOptions) {
          contentErrors.push(
            `${answerType === "radio" ? "Radio" : "Checkbox"} questions require at least ${minOptions} option${minOptions > 1 ? "s" : ""}`
          );
        }
      }
  ...
  // Memoize validation errors so tabHasErrors doesn't re-run full validation each call
  ...
  const tabHasErrors = useCallback(
    (tab: TabId): boolean => {
      switch (tab) {
        case "content":
          return !!validationErrors.content && validationErrors.content.length > 0;
        case "help":
          return !!validationErrors.help && validationErrors.help.length > 0;
        case "review":
          return !!validationErrors.notes;
        default:
          return false;
      }
    },
